...
sendTo = 'sydneynyongesa@gmail.com'
    ...
    while True:
        ...
        for c in cnt:
            if cv2.contourArea(c) > 800: # if contour area is less then 800 non-zero(not-black) pixels(white)
                ...
                emailContent = "The Intruder has been detected at: " + time.ctime()
        ...
        cv2.putText(frame, datetime.datetime.now().strftime('%A %d %B %Y %I:%M:%S%p'), 
            (10, frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX , 0.35, (0, 255, 0),1) # frame.shape[0] = hieght, frame.shape[1] = width,ssssssssssssss
